...
def macro_econ_node(state: BerkshireState):
    ...
    try:
        ...
        print(f"[Macro] Metrics available: {metrics_with_data}/5, score sum: {score_sum}")
            ...
            "analyst_signals": {
                "macro": {
                    ...
                    "horizon_alignment_note": (
                        f"Macro signals interpreted for {horizon_label(selected_horizon)}."
                    ),
                }
            }
        }
        ...
        print(f"[Macro] Unexpected error for {ticker}: {e}")
        return {
            "analyst_signals": {
                "macro": {
                    "rating": Rating.HOLD.value,
                    "confidence": 0.0,
                    "features": {"sector_performance": None, "market_trend": None},
                    "details": f"Error during macroeconomic analysis: {str(e)}",
                    "horizon_alignment_note": f"Macro analysis failed for {horizon_label(selected_horizon)}.",
                }
            }
        }
